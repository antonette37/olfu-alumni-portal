...
/* Partial include: profile edit can defer scripts so page HTML is not after a huge <script> block. */
$olfu_defer_header_scripts = !empty($olfu_defer_header_scripts);
$olfu_header_part = $olfu_header_part ?? 'full';
$olfu_header_render_styles = ($olfu_header_part === 'full' || $olfu_header_part === 'styles');
$olfu_header_render_nav = ($olfu_header_part === 'full' || $olfu_header_part === 'nav');
$olfu_header_render_widgets = ($olfu_header_part === 'full' || $olfu_header_part === 'tail');
$olfu_header_render_scripts = ($olfu_header_part === 'scripts') || ($olfu_header_part === 'full' && !$olfu_defer_header_scripts);
?>
<?php if ($olfu_header_render_styles) : ?>
<style>
...
/* Closed modals must not block page clicks (inline display:flex + opacity:0 still captures events). */
#floatingProfileModal.hidden,
#chatbotModal.hidden {
  display: none !important;
        ...
        <div style="position:relative;" id="msgWrap">
          <?php
            ...
            if ($stmt) {
              $stmt->bind_param('i', $user_id);
              $stmt->execute();
              $r = mysqli_stmt_fetch_assoc_compat($stmt);
              $unread_count = (int) ($r['uc'] ?? 0);
              $stmt->close();
            }
            $recent_messages = [];
            ...
            if ($stmt) {
              ...
              $stmt->execute();
              $recent_messages = mysqli_stmt_fetch_all_assoc_compat($stmt);
              $stmt->close();
            }
          ?>
          ...
          <div class="h-panel is-collapsed" id="messagesDropdown">
            ...
            <div class="h-panel-body">
              ...
              <?php else: foreach ($recent_messages as $msgItem):
                $toMe    = $msgItem['receiver_id'] == $user_id;
                $partner = $toMe ? $msgItem['sf'].' '.$msgItem['sl'] : $msgItem['rf'].' '.$msgItem['rl'];
                $unreadDot = (!$msgItem['is_read'] && $toMe);
              ?>
                <a href="al_messages.php" class="h-panel-item <?= $unreadDot ? 'unread' : '' ?>">
                  ...
                  <div style="flex:1;min-width:0;">
                    ...
                    <div class="h-notif-body" style="white-space:nowrap;overflow:hidden;text-overflow:ellipsis;"><?= htmlspecialchars($msgItem['subject'] ?? '') ?></div>
                    <div class="h-notif-time"><?= date('M d', strtotime($msgItem['created_at'])) ?></div>
                  </div>
...
<?php if ($olfu_header_render_widgets) : ?>
<?php if ($user_id):
  ...
  <div id="chatbotModal" class="cb-modal hidden" style="position:fixed;bottom:90px;right:88px;width:360px;height:520px;display:flex;flex-direction:column;z-index:160;transform:scale(0.92);opacity:0;">
    ...
    <div id="chatbotMessages" class="cb-messages">
      <div class="cb-bubble-wrap">
        ...
        <div class="cb-bubble bot">Hello! I can help with registration, Masterlist checks, New vs Legacy path, OTP, CPS ID, soft copy ID, and tracer questions. How can I assist you today?
          <div class="cb-suggestions">
            <button class="cb-suggestion-btn" onclick="cbSuggest('Which registration should I choose?')">New vs Legacy?</button>
            <button class="cb-suggestion-btn" onclick="cbSuggest('I did not get my OTP code')">OTP issue</button>
            <button class="cb-suggestion-btn" onclick="cbSuggest('How do I get my soft copy ID?')">Soft copy ID</button>
          </div>
        </div>
      </div>
    </div>
    ...
    <div class="fp-body">
      ...
      <div id="settingsTabContent" class="tab-content hidden">
        <div style="display:flex;flex-direction:column;gap:6px;">
          ...
          <button type="button" id="profileLogoutBtn" onclick="showLogoutModal()" class="fp-logout-btn"><i class="fas fa-sign-out-alt"></i> Logout</button>
        </div>
      </div>
    </div>
  </div>
...
<?php if ($olfu_header_render_scripts) : ?>
<!-- ====== SCRIPTS ====== -->
<script>
(function(){
  'use strict';
  ...
  function checkHeader() {
    ...
    } catch(e) { if (header) header.classList.remove('dark-bg'); }
  }
  ...
  document.addEventListener('DOMContentLoaded', function() {
    ...
    const p = document.getElementById('profileLogoutBtn');
    if (p) p.addEventListener('click', function(e){ e.preventDefault(); showLogoutModal(); });
...
<?php if ($user_id): ?>
  ...
  (function(){
    ...
    const botState = { awaitingIdClarification:false };
    ...
    function getResponse(raw) {
      const t = norm(raw);
      if (!t) return { answer:'Please type your question so I can help.', suggestions:[] };

      if (botState.awaitingIdClarification) {
        botState.awaitingIdClarification = false;
        if (hasAny(t, ['new','no card','first'])) {
          return { answer:'For <strong>New Applicant</strong>: register without an existing card number. After admin approval, your <strong>16-digit CPS ID</strong> is generated automatically and valid for <strong>3 years</strong>.', suggestions:['How do I get my soft copy ID?','How long does approval take?'] };
        }
        if (hasAny(t, ['legacy','existing','old card','already'])) {
          return { answer:'For <strong>Legacy Cardholder</strong>: choose legacy path, provide your existing alumni ID, and upload front/back card images. Your existing ID is preserved (no new ID generated).', suggestions:['What if my legacy ID is not accepted?','Can I still register if I graduated years ago?'] };
        }
        return { answer:'Please answer with either <strong>New Applicant</strong> or <strong>Legacy Cardholder</strong>.', suggestions:['New Applicant','Legacy Cardholder'] };
      }

      if ((t === 'id' || hasAny(t, ['alumni id','cps id','id number'])) && !hasAny(t, ['soft copy','validity','expire','download'])) {
        botState.awaitingIdClarification = true;
        return { answer:'I can help with IDs. Are you asking about <strong>New ID</strong>, <strong>Existing Physical Card</strong>, or <strong>Lost ID</strong>?', suggestions:['New ID','Existing Physical Card','Lost ID'] };
      }

      if (hasAny(t, ['record not found','registration failing','masterlist','not found','failed registration'])) {
        return { answer:'Registration is validated against the Registrar Masterlist using your <strong>Student Number</strong> and <strong>Birthday</strong>. Ensure both exactly match official records (no extra spaces or wrong format).', suggestions:['Which registration should I choose?','I graduated years ago, can I still register?'] };
      }
      if (hasAny(t, ['which registration','new vs legacy','legacy','existing cardholder','new applicant','graduated 2010','old graduate'])) {
        return { answer:'If you never had a physical alumni card, choose <strong>New Applicant</strong>. If you already have a physical card, choose <strong>Legacy Cardholder</strong> to digitize your existing ID.', suggestions:['What is a CPS ID?','How long does approval take?'] };
      }
      if (hasAny(t, ['otp','did not get','code','resend','security'])) {
        return { answer:'OTP codes are sent to your registered personal email. Check <strong>Spam/Junk</strong>, then tap <strong>Resend</strong> after 60 seconds. OTP is valid for about <strong>15 minutes</strong>.', suggestions:['What if I changed my email?','How do I contact support?'] };
      }
      if (hasAny(t, ['what is cps id','cps id','16-digit','cps'])) {
        return { answer:'The CPS ID is your official <strong>16-digit OLFU alumni identifier</strong>. For new users it is generated upon approval, and it follows a <strong>3-year validity</strong> cycle for updated records.', suggestions:['Why does my ID expire?','How do I get my soft copy ID?'] };
      }
      if (hasAny(t, ['soft copy','digital id','download id','png'])) {
        return { answer:'After admin approval, your digital ID is emailed automatically. You can also view and download it anytime from your <strong>Alumni ID Card</strong> page.', suggestions:['Can I use soft copy for campus entry?','Why does my ID expire?'] };
      }
      if (hasAny(t, ['expire','expiry','validity','renew'])) {
        return { answer:'Alumni IDs are valid for <strong>3 years</strong> to keep tracer and accreditation records current. Renewal usually requires profile/employment update verification.', suggestions:['What is tracer study?','How do I update my profile?'] };
      }
      if (hasAny(t, ['tracer','career','why should i fill','job board','job openings'])) {
        return { answer:'Tracer and career updates help OLFU track graduate outcomes for accreditation and curriculum improvement. You can browse opportunities in <strong>Career Center</strong>.', suggestions:['How do I find job openings?','How do I update employment status?'] };
      }
      if (hasAny(t, ['change photo','update photo'])) {
        return { answer:'Yes, you can update your profile photo. For ID-related updates, admin re-verification may be required before changes appear on your official alumni card.', suggestions:['How do I update my profile?','How do I contact support?'] };
      }
      if (hasAny(t, ['support','coordinator','admin','contact'])) {
        return { answer:'You can message the Alumni Coordinator directly via <a href="al_contact.php">Contact Support</a>.', suggestions:['Open contact page','How long does approval take?'] };
      }
      return { answer:'I could not find an exact match yet. You can ask about <strong>Masterlist</strong>, <strong>New vs Legacy</strong>, <strong>OTP</strong>, <strong>CPS ID</strong>, <strong>Soft Copy</strong>, or use <a href="al_contact.php">Contact Support</a>.', suggestions:['Which registration should I choose?','I did not get my OTP code','What is a CPS ID?'] };
    }
    ...
    function addMsg(text, isBot=false, suggestions=[]) {
      ...
      bubble.innerHTML = isBot ? String(text) : esc(text);
