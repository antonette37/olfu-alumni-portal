-- Alternative job listings table without foreign key constraint
-- This version removes the foreign key constraint to avoid dependency issues

-- Drop existing table if it exists
DROP TABLE IF EXISTS job_listings_new;

-- Create new job_listings table WITHOUT foreign key constraint
CREATE TABLE job_listings_new (
    ...
    created_by INT DEFAULT NULL, -- No foreign key constraint
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;

-- Insert sample job listings (created_by can be NULL or any value)
...
DROP TABLE IF EXISTS job_listings_old;
